...
class EbaySpider(scrapy.Spider):
    ...
    def parse_info(self, response):
        # Get all the spans with class "ux-textspans"
        spans = response.xpath('//span[@class="ux-textspans"]')
        
        # Iterate through each span to find the one with "ISBN" and extract the ISBN value
        for span in spans:
            label_text = span.xpath('./text()').get()
            if "ISBN" in label_text:
                isbn = span.xpath('./following::span[1]/text()').get()

                # You can then do whatever you want with the extracted ISBN value, for example, print it
                print("ISBN:", isbn)
